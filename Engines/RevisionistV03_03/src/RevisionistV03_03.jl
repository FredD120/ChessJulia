...
function best_move(board::Boardstate,T_MAX,logging=false)
    ...
    if logging
        println("Best move = $(UCImove(best_move)). Move score = $(logger.best_score). Evaluated $(logger.cum_nodes) moves. Reached depth $((logger.cur_depth)). Time taken: $(round(δt,sigdigits=6))s.)")
        if logger.stopmidsearch
