...
Logger() = Logger(0,0,0,Dict{UInt8,UInt64}(),0,0,0,0,0,false)
...
function minimax(board,player,α,β,depth,info::SearchInfo,logger::Logger)
    ...
    elseif depth <= MINDEPTH
        ...
        moves = @log_time logger.movegenδt generate_moves(board)
        for move in moves
            t = time()
...
function best_move(board::Boardstate,T_MAX,logging=false)
    ...
    if logging
        ...
        println("Time: $(δt)s. Evaluation: $(Int(round(100*logger.evalδt/δt)))%. Movegen: $(Int(round(100*logger.movegenδt/δt)))%. Move ordering: $(Int(round(100*logger.moveorderδt/δt)))%. Make/Unmake: $(Int(round(100*logger.makeδt/δt)))%.")
